...
export default {
  ...
  created() {
    ...
    if (this.$route.query.id) {
      var cc = (this.$route.query.id - 1)
      this.istype = this.$route.query.id
      // this.activeName = this.$route.query.id
      console.log("this.istypethis.istypethis.istype:", this.istype)
      console.log("activeNameactiveName:", this.activeName)
    } else {
      this.istype = 0
    }
  ...
  methods: {
    ...
    fyfunction(val) {
      ...
      info('').then(res => {
        ...
        xbqv2(options).then(res => {
          if (res.code === 100) {
            console.log("xbqcolumns:", res.data)
            this.tabinfo = res.data.columnInfo
            this.menulist = res.data.notes.data
            console.log("2222222222222", this.menulist)
    ...
    setcartoon(id, note_type, detail_url) {
      console.log("12:", id)
      console.log("122222:", note_type)
      console.log("2222222:", detail_url)
      if (note_type == 1) {
        this.$router.push({
      ...
      // this.$router.push({ path: '/videoLibrary/Cartoon' },query : { happyid:id})
      // this.$router.push({ path: '/videoLibrary/Cartoon', query: { happyid: id}})
    },
    handleClose(done) {
      this.MXcallzz = ''
    ...
    openvideos(items) {
      console.log("llllllllllll:", items)
      if (!items.video) {
        return false
      }
    ...
    openvideoauto(items) {
      console.log("kkkkkkkkkkkkkkk:", items)
    ...
    handleClick(tab, event) {
      console.log('1212', tab.$vnode.key)
      this.currentPage1 = 1
      ...
      info('').then(res => {
        ...
        xbqv2(options).then(res => {
          if (res.code === 100) {
            ...
            console.log("3333333", this.menulist)
            console.log("this.istypethis.istypethis.istype:", this.istype)
            console.log("cccccc：", cc)
            console.log("cccccc：", cc.length)
            var that = this
...
<style lang="scss" >
@import "../../static/styles/videoLibrary/videoLibrary.scss"
</style>
