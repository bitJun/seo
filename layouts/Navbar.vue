<template>
<div id="nav_list" class="navbar">
  <div class="right-menu">
    <!--已经登录-->
    <!--      1 登录立即刷新-->
    <el-dropdown v-if="islogin==1" class="avatar-container" trigger="hover">
      <div class="avatar-wrapper">
        <!--用户头像-->
        <img :src="avatar_urls" class="user-avatar" @click="setonemuver" style="border-radius: 50%">
      </div>
      <el-dropdown-menu slot="dropdown" class="user-dropdown">
        <!-- <nuxt-link :to="json.path"> -->
        <!-- </nuxt-link> -->
        <router-link :to="{path:'/Mycurriculum',query:{Mycurriculumid:9}}">
          <el-dropdown-item>
            消息中心
          </el-dropdown-item>
        </router-link>
        <router-link :to="{path:'/Mycurriculum',query:{Mycurriculumid:3}}">
          <el-dropdown-item>
            我的订单
          </el-dropdown-item>
        </router-link>
        <router-link :to="{path:'/Mycurriculum',query:{Mycurriculumid:5}}">
          <el-dropdown-item>
            我的地址
          </el-dropdown-item>
        </router-link>
    ...
    <!--未登录-->
    <el-dropdown v-if="islogin==0" class="avatar-container">
      <div class="avatar-wrapper">
        <!--用户头像-->
  ...
  <!--    登录刷新-->
  <div v-if="islogin == 1" class="right-menu mycourse" @click="Mycurriculum">
    <img src="/images/lan.png" alt="">
    我的课程
  </div>
  <!--    全程缓存-->
  <!-- <div v-if=" token && token!==''" class="right-menu mycourse" @click="Mycurriculum">
      <img src="/images/lan.png" alt="">
      我的课程
    </div> -->
  <div>
          ...
          <el-form-item prop="phone">
            ...
            <div>
              ...
              <div style="text-align: center">
                <el-checkbox style="margin-right: 0;margin-left: -50px;border-radius: 50%" @change="handleChange">
                  登录即代表同意
                </el-checkbox>
                <span @click="setbook()" style="cursor:pointer;">《犀鸟公考用户协议》</span>
  ...
  <!-- 绑定用户登录 -->
  <el-dialog :visible.sync="userbind" :append-to-body="true" :before-close="handleClose" :close-on-click-modal="false" custom-class="dialog_list_nav">
    <div>
      <img class="xiniaologo" src="/images/icon_login.png" alt="">
      <el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm" label-width="60px" style="margin-top: 30%">
        <el-form-item prop="phone">
          ...
          <div>
            ...
            <div style="text-align: center">
              ...
              <span @click="setbook()" style="cursor:pointer;">《犀鸟公考用户协议》</span>
              <div style="border: transparent;width: 60%;background: #33CCD3;border-radius: 20px;color: #ffffff;margin-left: 60px;margin-top: 10px;cursor:pointer;" @click="bindphone">绑定</div>
            </div>
          </div>
        </el-form-item>
      </el-form>
...
export default {
  ...
  // },
  mounted() {
    let usercode = this.GetQueryValue('code')
    this.usercode = usercode
    let cc = this.usercode
    let loginlists = ''
    if (typeof window == 'object') {
      loginlists = localStorage.getItem('nickname')
    }
    if (loginlists) {
      console.log("用户已登录")
      this.islogin = 1
    ...
    if (typeof window == 'object') {
      ...
      this.avatar_urls = window.localStorage.getItem('avatar_url')
    }
  },
  methods: {
    toggleSideBar() {
    ...
    wxlogin() {
      info('').then(res => {
        ...
        login(options).then(res => {
          console.log("11112223333:", res)
          if (res.code === 100) {
            this.onlogin = true
            this.dialogVisible = false
            this.Generaltable = res.data
            if (!this.Generaltable.phone) {
              this.userbind = true
              this.$alert('请绑定手机号！')
            } else {
              this.$alert('登陆成功')
          ...
          phoneLogin(options).then(res => {
            if (res.code === 100) {
              ...
              this.$nextTick(() => {
                ...
                this.userbind = false;
                this.islogin = 1;
                this.avatar_urls = this.Generaltable.avatar_url;
                if (typeof window == 'object') {
...
.el-button--success {
  background-color: #33CCD3 !important;
  border-color: #33CCD3 !important;
}
</style>
