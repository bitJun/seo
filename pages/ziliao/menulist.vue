...
<div class="app-container">
  <ul class="left-img">
    <li @mouseenter="tablezzz1" style="position: relative" @mouseleave="tablesa">
      <div v-if="menulist.is_collect === 0 && shoucang === null" class="shoucangimg" @click="checkshoucang"></div>
      <img v-else src="/images/liicon/shoucang-a.png" @click="checkedshoucang" style="width: 48px;height: 48px;cursor:pointer;">
      <img v-show="zzz1" src="/images/zzz2.png" style="width: 112px;height: 44px;position: absolute;left: -110px;top: 0;z-index: 9">
    </li>
    <li class="scroll_bottom" @mouseenter="tablezzz2" style="position: relative" @mouseleave="tablesa">
      <img v-show="zzz2" src="/images/zzz1.png" style="width: 112px;height: 44px;position: absolute;left: -110px;top: 0;z-index: 9;">
    </li>
  </ul>
  <div style="width: 1169px;height: 100%;margin:auto;background: #ffffff;padding-bottom: 50px;padding-top: 50px;border-radius: 10px">
    <div v-if="abc === 0 && menulist.exam_number !== 0">
      <div style="width: 745px;margin: 0 4% 1%;position: relative;display: inline-block">
        <img src="/images/yaodian.png" style="position: absolute;left: -24px;top: -28px;z-index: 99;width: 165px;height: 71px">
        <ul style="line-height: 34px;padding-top: 40px;color: #666666;background: #F1F5F8;width: 100%;padding-bottom: 20px;border-radius: 10px">
          <!--            地区可以容纳38个字-->
          <li v-if="menulist.exam_area !== ''">招考地区：<span class="over-list" style="width: 600px">{{ menulist.exam_area }}</span></li>
          <li v-if="menulist.exam_department !== ''">招考部门：<span>{{ menulist.exam_department }}</span></li>
          <li v-if="menulist.exam_number !== ''">招考人数：<span>{{ menulist.exam_number }}</span></li>
          <li v-if="menulist.exam_subject !== ''">考试科目：<span>{{ menulist.exam_subject }}</span></li>
          <li v-if="menulist.sign_time !== ''">报名时间：<span>{{ menulist.sign_time }}</span></li>
          <li v-if="menulist.mark_time !== ''">准考证打印时间：<span>{{ menulist.mark_time }}</span></li>
          <li v-if="menulist.exam_time !== ''">考试时间：<span>{{ menulist.exam_time }}</span></li>
        </ul>
      </div>
    ...
    <div style="display: flex;justify-content: space-between;padding: 1% 0 1% 1%">
      <div style="display: flex;margin-left: 2%;">
        <div style="line-height: 36px;width: 800px;padding: 0 2%">
          <div style="font-weight: bold;color: #333333;font-size: 24px;word-break:break-all;">{{ menulist.title }}</div>
          <div style="display: flex;justify-content: space-between;color: #9AABB8">
            <div v-if="menulist.regdate" style="font-size: 14px"><img src="/images/icon/riqi.png" alt="" style="width: 12px;height: 12px;margin-right: 10px;vertical-align: middle;color: #9AABB8;margin-bottom: 2px">{{ (menulist.regdate).replace(/\-/g,".") }}<span style="margin-left: 30px">来源：{{menulist.author}}</span>
            </div>
          ...
          <div v-if="acaa.length" style="background: #F8F8F8;padding-left: 35px;padding-bottom: 10px;padding-top: 10px;border-radius: 4px">
            <div style="color: #333333;font-size: 18px">·附件·</div>
            <div v-for="(item,index) in menulist.annex" v-bind:key="index.id">
              <div style="color: #33CCD3;padding: 4px 0;height: 40px" class="over-list" @click="setInmg(item)">附件{{index + 1}}： {{item.name}}</div>
            </div>
          </div>
        </div>
      </div>
      <div v-if="menulist.exam_number === 0">
        <div style="width: 299px;margin-right: 20px;object-fit:cover;margin-bottom: 30px" v-for="(item,index) in banners" v-bind:key="index.url">
          <img :src="item.img_url" alt="" @click="setdetails(item)" style="width: 100%;border-radius: 6px;height: 150px;object-fit:cover">
        </div>
      </div>
    </div>
    <div>
      ...
      <div v-for="(item,index) in takeset" v-bind:key="index.url" style="border-bottom: 1px solid #f5f5f5;padding-bottom: 25px">
        <div style="margin: 2% 4% 0 4%;display: flex;justify-content: space-between;">
          <div style="display: flex;padding-bottom: 2%;line-height: 30px">
            <img :src="item.avatar_url" alt="" style="width: 40px;height: 40px;margin-right: 20px;border-radius: 25px;">
            <div>
              <div style="color: #999999">{{ item.nickname }}
                <img v-if="item.sex === 1" src="/images/icon/comment_nan_cion.png" style="vertical-align: middle">
                <img v-if="item.sex === 2" src="/images/icon/comment_nv_icon.png" alt="" style="vertical-align: middle">
  ...
  <!--    底部评论栏-->
  <div class="bottomsss" style="width: 1169px;margin: 2% auto;background: #ffffff;border-radius: 10px;">
    <div style="display: flex;justify-content: space-between;color: #9AABB8;height: 70px;line-height: 70px;padding: 0 40px;">
      ...
      <div style="position: relative">
        <input type="text" placeholder="为了轻松上岸，我准备发个言" style="width: 810px;border-radius: 30px;border: 1px solid #9AABB8;padding: 12px 26px;outline:none" v-model.trim="inputup">
...
export default {
  ...
  created() {
    ...
    this.comebaby = this.$route.query.detail
    ...
    info('').then(res => {
      ...
      referInfov2(options).then(res => {
        if (res.code === 100) {
          this.menulist = res.data
          this.acaa = res.data.annex
          this.abc = 0
        } else {
      ...
      referCommentv2(options).then(res => {
        if (res.code === 100) {
    ...
    let loginlists;
    if (typeof window == 'object') {
      loginlists = window.localStorage.getItem('nickname')
    }
    console.log("loginlistsloginlistsloginlists:", loginlists)
    if (loginlists) {
      console.log("用户已登录")
      this.islogin = 1
    } else {
  ...
  mounted() {
    $(document).ready(function () {
      $('.scroll_bottom').on('click', function () {
        $('html,body').animate({
          scrollTop: $('.bottomsss').offset().top
        }, 800)
      })
  ...
  methods: {
    ...
    setinputvalue() {
      //   var loginlists = localStorage.getItem('nickname')
      // console.log("loginlistsloginlistsloginlists:",loginlists)
      // if(loginlists){
      //   console.log("用户已登录")
      //   this.islogin = 1
    ...
    dianzan(item) {
      if (this.token === '' || this.token === null) {
        ...
        info('').then(res => {
          ...
          commentPraisev2(options).then(res => {
            if (res.code === 100) {
              info('').then(res => {
                const timestamp = res
                const options = {
...
<style lang="scss">
@import "../../static/styles/nested/menu/menulist.scss";
</style>
